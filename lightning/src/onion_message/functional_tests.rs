...
fn one_hop() {
	...
	nodes[0].messenger.send_onion_message(vec![], Destination::Node(nodes[1].get_node_pk())).unwrap();
...
fn two_unblinded_hops() {
	...
	nodes[0].messenger.send_onion_message(vec![nodes[1].get_node_pk()], Destination::Node(nodes[2].get_node_pk())).unwrap();
	pass_along_path(nodes, None);
}
...
fn two_unblinded_two_blinded() {
	...
	let secp_ctx = Secp256k1::new();
	let blinded_route = BlindedRoute::new::<EnforcingSigner, _, _>(vec![nodes[3].get_node_pk(), nodes[4].get_node_pk()], &*nodes[4].keys_manager, &secp_ctx).unwrap();

	nodes[0].messenger.send_onion_message(vec![nodes[1].get_node_pk(), nodes[2].get_node_pk()], Destination::BlindedRoute(blinded_route)).unwrap();
	pass_along_path(nodes, None);
}
...
fn three_blinded_hops() {
	...
	let secp_ctx = Secp256k1::new();
	let blinded_route = BlindedRoute::new::<EnforcingSigner, _, _>(vec![nodes[1].get_node_pk(), nodes[2].get_node_pk(), nodes[3].get_node_pk()], &*nodes[3].keys_manager, &secp_ctx).unwrap();

	nodes[0].messenger.send_onion_message(vec![], Destination::BlindedRoute(blinded_route)).unwrap();
	pass_along_path(nodes, None);
}
...
fn invalid_blinded_route_error() {
	...
	let secp_ctx = Secp256k1::new();
	let mut blinded_route = BlindedRoute::new::<EnforcingSigner, _, _>(vec![node2.get_node_pk(), node3.get_node_pk()], &*node3.keys_manager, &secp_ctx).unwrap();
