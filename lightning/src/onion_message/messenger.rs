...
/// # let (hop_node_id2, hop_node_id3, hop_node_id4) = (hop_node_id1.clone(), hop_node_id1.clone(),
/// hop_node_id1.clone());
...
/// let hops = vec![hop_node_id3, hop_node_id4, your_node_id];
...
pub enum SendError {
	...
	TooBigPacket,
	/// The provided [Destination] was an invalid [super::BlindedRoute], due to having 0 blinded hops.
	MissingBlindedHops,
}
...
fn packet_payloads_and_keys<T: secp256k1::Signing + secp256k1::Verification>(
	...
	let (mut intro_node_id_blinding_pt, num_blinded_hops) = if let Destination::BlindedRoute(BlindedRoute {
		...
		(Some((introduction_node_id.clone(), blinding_point.clone())), blinded_hops.len()) } else { (None, 0) };
