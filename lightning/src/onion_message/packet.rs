...
use crate::blinded_path::message::{BlindedMessagePath, DummyTlvs, ForwardTlvs, NextMessageHop, ReceiveTlvs, UnauthenticatedDummyTlvs};
...
impl<T: OnionMessageContents> Writeable for (Payload<T>, [u8; 32]) {
	fn write<W: Writer>(&self, w: &mut W) -> Result<(), io::Error> {
		match &self.0 {
			...
			Payload::Dummy(DummyControlTlvs::Blinded(encrypted_bytes)) => {
				_encode_varint_length_prefixed_tlv!(w, { (4, *encrypted_bytes, required_vec) })
			},
			Payload::Receive {
