...
//! Lighning message signing and verification lives here. This tools can be used to sign messages using the node's
//! secret so receivers are sure that they come from you. You can also use this to verify that a given message comes
//! from a specific node.
//! Furthermore, this tools can be used to sign / verify messages using ephemeral keys not tight to node's identities.
//!
//! Note this is not parts of the specs, but follows lnd's signing and verifying protocol, which can is defined as follows:
//!
...
//!
//! This implementation is compatible with boths lnd's and c-lightning's
//!
...
/// A receiver knowing the PublicKey (e.g. the node's id) and the message can be sure that the signature
/// was generated by the claimer. Signatures are EC recoverable, meaning that given the message and the signature
/// the PublicKey of the signer can be extracted.
...
mod test {
    ...
    fn test_verify_ground_truth_ish() {
        // There are no standard tests vectors for Sign/Verify, using the same tests vectors as c-lighning to see if they are compatible.
