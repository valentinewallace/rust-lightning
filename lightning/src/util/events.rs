...
pub enum Event {
	...
	PaymentReceived {
		...
		amt: u64,
		/// Information for claiming this received payment.
		receipt: ReceiveInfo,
	},
...
impl Writeable for Event {
	fn write<W: Writer>(&self, writer: &mut W) -> Result<(), ::std::io::Error> {
		match self {
			...
			&Event::PaymentReceived { ref payment_hash, ref amt, ref receipt } => {
				1u8.write(writer)?;
				let mut payment_secret = None;
				let mut user_payment_id = None;
				let payment_preimage;
				match &receipt {
					ReceiveInfo::Invoice { payment_preimage: preimage, payment_secret: secret, user_payment_id: id }=> {
		...
		match Readable::read(reader)? {
			...
			1u8 => {
				let f = || {
					...
					let receipt = match payment_secret {
						Some(secret) => ReceiveInfo::Invoice {
							payment_preimage,
