...
struct RouteGraphNode {
	...
	// The maximum value that may be passed via this node.
	// The upper bound is minimum of:
	// - how much is needed for a path being constructed
	// - how much value can channels following this node (up to the destination) can contribute, considering fees
		...
		( $chan_id: expr, $src_node_id: expr, $dest_node_id: expr, $directional_info: expr, $capacity_sats: expr, $chan_features: expr, $next_hops_fee_msat: expr,
		   $next_hops_value_contribution: expr ) => {
			...
			if let Some(available_value_contribution_msat) = available_liquidity_msat.checked_sub($next_hops_fee_msat) {
				...
				let minimal_value_contribution_msat: u64 = (recommended_value_msat - already_collected_value_msat) / 20;
			...
			if pubkey == *our_node_id {
				...
				for (i, payment_hop) in payment_path.hops.iter().enumerate() {
					...
					if i != payment_path.hops.len() - 1 {
						// For the last hop the fee is 0, so we don't have to add it here.
						spent_on_hop_msat += payment_hop.get_fee_paid_msat();
					}
