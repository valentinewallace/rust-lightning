...
pub struct BlindedTail {
	...
	/// The CLTV delta for the recipient of the payment.
...
impl Path {
	...
	pub fn final_cltv_expiry_delta(&self) -> u32 {
		match &self.blinded_tail {
			Some(blinded_tail) => blinded_tail.final_cltv_expiry_delta,
			None => self.hops.last().map_or(0, |hop| hop.cltv_expiry_delta)
		}
	}
}
