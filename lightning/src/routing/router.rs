...
mod tests {
	...
	fn previously_used_liquidity_violates_max_htlc() {
		// Test that if a candidate hop does not have enough contribution amount to cover the next hop's
		// min_htlc plus fees, we will not consider that candidate. In this case, the candidate does not
		// have enough due to a previous path taking up some of its liquidity. Previously we would
		// construct an invalid path and hit a debug panic asserting that the used liquidity for a hop
		// was less than its available liquidity limit.
		let secp_ctx = Secp256k1::new();
		let logger = Arc::new(ln_test_utils::TestLogger::new());
		let network_graph = Arc::new(NetworkGraph::new(Network::Testnet, Arc::clone(&logger)));
		let scorer = ln_test_utils::TestScorer::new();
