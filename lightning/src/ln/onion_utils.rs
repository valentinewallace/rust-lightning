	...
	construct_onion_keys_callback(secp_ctx, &path.hops, session_priv, |shared_secret, _, _, route_hop, route_hop_idx| {
		...
		if error_code & BADONION == BADONION {
			...
			let chan_update_opt = {
				...
				let mut update_slice = err_packet.failuremsg.get(debug_field_size+2..debug_field_size+4)
				...
				// not. The BOLTs have now been updated to indicate that they *are* included, but many
				// nodes still send messages without the type bytes, so we support both here.
				// TODO: Switch to hard require the type prefix, as the current permissiveness introduces
				// the (although small) possibility that we fail to decode legitimate channel updates that
				...
				} else if update_present {
					log_trace!(logger, "Failure provided features a channel update without type prefix. Deprecated, but allowing for now.");
				}
				if let Ok(upd) = msgs::ChannelUpdate::read(&mut Cursor::new(&update_slice)) {
					Some(upd)
				} else {
					if update_present {
						// If the channel_update had a non-zero length (i.e. was present) but we couldn't read
						// it, treat it as a total node failure.
			...
			let is_chan_update_invalid = chan_update_opt.as_ref().map_or(false, |upd| {
				match error_code & 0xff {
					7 => false,
					11 => amt_to_forward > upd.contents.htlc_minimum_msat,
					12 => amt_to_forward
						.checked_mul(upd.contents.fee_proportional_millionths as u64)
						.map(|prop_fee| prop_fee / 1_000_000)
						.and_then(|prop_fee| prop_fee.checked_add(upd.contents.fee_base_msat as u64))
						.map(|fee_msats| route_hop.fee_msat >= fee_msats)
						.unwrap_or(false),
					13 => route_hop.cltv_expiry_delta as u16 >= upd.contents.cltv_expiry_delta,
					14 => false, // expiry_too_soon; always valid?
					20 => upd.contents.flags & 2 == 0,
			...
			} else if let Some(upd) = chan_update_opt {
				// Make sure the ChannelUpdate contains the expected short channel id.
				if failing_route_hop.short_channel_id == upd.contents.short_channel_id {
					short_channel_id = Some(failing_route_hop.short_channel_id);
				} else {
					log_info!(logger, "Node provided a channel_update for which it was not authoritative, ignoring.");
				}
				network_update = Some(NetworkUpdate::ChannelUpdateMessage { msg: upd })
			} else if network_update.is_none() {
