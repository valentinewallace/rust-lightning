...
/// identical, we use this enum to allow `construct_onion_packet_with_init_noise` to return both
/// types.
enum Packet {
	/// This packet is for a payment.
	Payment(msgs::OnionPacket),
	/// This packet is for an onion message.
...
fn construct_onion_packet_with_init_noise<HD: Writeable>(mut payloads: Vec<HD>, onion_keys: Vec<OnionKeys>, mut packet_data: PacketData, associated_data: Option<&PaymentHash>) -> Packet {
	...
	match packet_data {
		PacketData::Payment(arr) => {
			Packet::Payment(msgs::OnionPacket {
				version: 0,
		...
		PacketData::Message(vec) => {
			Packet::Message(onion_message::Packet {
				version: 0,
...
enum NextPacketBytes {
	/// The next hop's packet is for a payment, thus is fixed size.
	Payment([u8; 20*65]),
	/// The next hop's packet is for an onion message, thus is variable-length.
	Message(Vec<u8>),
}
...
mod tests {
	...
	fn onion_vectors() {
		...
		let packet: msgs::OnionPacket = super::construct_onion_packet_with_init_noise(payloads, onion_keys, super::PacketData::Payment([0; super::ONION_DATA_LEN]), Some(&PaymentHash([0x42; 32]))).try_into().unwrap();
