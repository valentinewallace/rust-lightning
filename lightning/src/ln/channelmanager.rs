...
pub(super) struct BlindingState {
	...
	fn construct_fwd_pending_htlc_info(
		...
		Ok(PendingHTLCInfo {
			routing: PendingHTLCRouting::Forward {
				...
				short_channel_id,
				blinded: next_blinding_pt.map(|bp| BlindingState {
					next_hop_blinding_point: bp,
					we_are_intro_node: intro_node_blinding_point.is_some(),
				}),
			},
									...
									HTLCForwardInfo::AddHTLC(PendingAddHTLCInfo {
										...
										if let PendingHTLCRouting::Forward { ref onion_packet, .. } = routing {
											...
											if phantom_pubkey_res.is_ok() && fake_scid::is_valid_phantom(&self.fake_scid_rand_bytes, short_chan_id, &self.genesis_hash) {
												...
												let next_hop = match onion_utils::decode_next_payment_hop(phantom_shared_secret, &onion_packet.hop_data, onion_packet.hmac, payment_hash, None, &self.node_signer) {
													Ok(res) => res,
													Err(onion_utils::OnionDecodeErr::Malformed { err_msg, err_code }) => {
		...
		if let Some(height) = height_opt {
			...
			intercepted_htlcs.retain(|_, htlc| {
				if height >= htlc.forward_info.outgoing_cltv_value - HTLC_FAIL_BACK_BUFFER {
					let (requested_forward_scid, blinded) = match htlc.forward_info.routing {
						PendingHTLCRouting::Forward { short_channel_id, blinded, .. } =>
							(short_channel_id, blinded),
						_ => unreachable!(),
					};
...
impl_writeable_tlv_based!(BlindingState, {
