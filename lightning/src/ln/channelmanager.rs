			...
			let routing = match next_hop_data.format {
				...
				msgs::OnionHopDataFormat::FinalNode { payment_data, keysend_preimage } => {
					if payment_data.is_some() && keysend_preimage.is_some() {
						...
						// We need to check whether the keysend preimage matches the payment hash here to
						// prevent probing attacks wherein an attacker can use the payment processing time of a
						// bogus keysend to determine whether we had a pending inbound payment with the same
						// payment hash.
