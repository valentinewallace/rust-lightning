					...
					'next_forwardable_htlc: for forward_info in pending_forwards.drain(..) {
						match forward_info {
							HTLCForwardInfo::AddHTLC(PendingAddHTLCInfo {
								...
								let (
									cltv_expiry, onion_payload, payment_data, payment_context,
									phantom_shared_secret, mut onion_fields, invoice_request_opt
								) = match routing {
									PendingHTLCRouting::Receive {
										...
										(incoming_cltv_expiry, OnionPayload::Invoice { _legacy_hop_data },
											Some(payment_data), payment_context, phantom_shared_secret, onion_fields, None)
									},
									PendingHTLCRouting::ReceiveKeysend {
										...
										(incoming_cltv_expiry, OnionPayload::Spontaneous(payment_preimage),
											payment_data, payment_context, None, onion_fields, invoice_request)
									},
									_ => {
										...
										match claimable_htlc.onion_payload {
											...
											OnionPayload::Spontaneous(preimage) => {
												let purpose = if let Some(PaymentContext::AsyncBolt12Offer(
													AsyncBolt12OfferContext { offer_id, offer_nonce }
												)) = payment_context {
													...
													let (payment_preimage_opt, min_final_cltv_expiry_delta) =
														match inbound_payment::verify(
															...
															Some(offer_id), &self.inbound_payment_key, &self.logger
														) {
															Ok(result) => result,
															Err(()) => {
	...
	L::Target: Logger,
{
	...
	pub fn create_static_invoice_builder_for_async_receive_offer<'a>(
		&self, offer: &'a Offer, nonce: Nonce, relative_expiry: Option<Duration>
	) -> Result<StaticInvoiceBuilder<'a>, Bolt12SemanticError> {
		let payment_context = PaymentContext::AsyncBolt12Offer(
			AsyncBolt12OfferContext { offer_id: offer.id(), offer_nonce: nonce }
		);
		...
		let payment_secret = inbound_payment::create_for_static_invoice(
		...
		let context = MessageContext::AsyncPayments(
			AsyncPaymentsContext::InboundPayment { offer_id: offer.id() }
		);
		let async_receive_message_paths = self.create_blinded_paths(context)
			.map_err(|()| Bolt12SemanticError::MissingPaths)?;

		StaticInvoiceBuilder::for_offer_using_derived_keys(
			offer, payment_paths, async_receive_message_paths, created_at, &self.inbound_payment_key,
			nonce, &self.secp_ctx
		)
	}
