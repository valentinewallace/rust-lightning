...
fn test_channel_reserve_holding_cell_htlcs() {
	...
	match events[0] {
		Event::PaymentReceived { ref payment_hash, ref receipt, amt } => {
			...
			match &receipt {
				...
				_ => panic!("expected ReceiveInfo::Invoice")
			}
		},
		_ => panic!("Unexpected event"),
	}
...
fn test_drop_messages_peer_disconnect_dual_htlc() {
	...
	match events_5[0] {
		Event::PaymentReceived { ref payment_hash, ref receipt, .. } => {
...
fn test_preimage_storage() {
	...
	assert_eq!(events.len(), 1);
	match events[0] {
		Event::PaymentReceived { ref receipt, .. } => {
			match &receipt {
				ReceiveInfo::Invoice { payment_preimage, user_payment_id, .. } => {
...
fn test_secret_timeout() {
	...
	assert_eq!(events.len(), 1);
	match events[0] {
		Event::PaymentReceived { receipt: ReceiveInfo::Invoice { payment_preimage, payment_secret, user_payment_id }, .. } => {
			assert!(payment_preimage.is_none());
			assert_eq!(user_payment_id, 42);
