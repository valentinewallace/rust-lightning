...
fn test_no_txn_manager_serialize_deserialize() {
	...
	let data_persister: test_utils::TestChanDataPersister;
	...
	data_persister = test_utils::TestChanDataPersister{};
	new_chain_monitor = test_utils::TestChainMonitor::new(Some(nodes[0].chain_source), nodes[0].tx_broadcaster.clone(), &logger, &fee_estimator, &data_persister);
...
fn test_key_derivation_params() {
	...
	let chain_monitor = test_utils::TestChainMonitor::new(Some(&chanmon_cfgs[0].chain_source), &chanmon_cfgs[0].tx_broadcaster, &chanmon_cfgs[0].logger, &chanmon_cfgs[0].fee_estimator, &chanmon_cfgs[0].data_persister);
...
fn test_data_loss_protect() {
	...
	keys_manager = test_utils::TestKeysInterface::new(&nodes[0].node_seed, Network::Testnet);
	data_persister = test_utils::TestChanDataPersister{};
	monitor = test_utils::TestChainMonitor::new(Some(&chain_source), &tx_broadcaster, &logger, &fee_estimator, &data_persister);
	node_state_0 = {
		let mut channel_monitors = HashMap::new();
...
fn test_update_err_monitor_lockdown() {
	...
	let watchtower = {
		...
		let watchtower = test_utils::TestChainMonitor::new(Some(&chanmon_cfgs[0].chain_source), &chanmon_cfgs[0].tx_broadcaster, &logger, &chanmon_cfgs[0].fee_estimator, &chanmon_cfgs[0].data_persister);
