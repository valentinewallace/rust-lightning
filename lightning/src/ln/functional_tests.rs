...
fn test_fee_spike_violation_fails_htlc() {
	...
	println!("VMW: commit_tx_keys: {:?}", commit_tx_keys);
	...
	println!("VMW: commit_tx_keys.revocation_key {}, commit_tx_keys.a_delayed_payment_key: {}, commit_tx_keys.", commit_tx_keys.revocation_key, commit_tx_keys.a_delayed_payment_key);
	...
	println!("VMW: in test: commit tx: {:?}", commit_tx);
	let res = {
	...
	// let res = local_chan_keys.sign_remote_commitment(feerate_per_kw, &commit_tx, &commit_tx_keys, &[&accepted_htlc_info],
		// BREAKDOWN_TIMEOUT, &secp_ctx).unwrap();
	...
	assert_eq!(events.len(), 1);
	let update_fail_msg: msgs::UpdateFailHTLC = {
		match events[0] {
