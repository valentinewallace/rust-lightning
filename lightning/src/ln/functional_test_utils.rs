...
pub struct TestChanMonCfg {
	...
	pub data_persister: test_utils::TestChanDataPersister,
...
impl<'a, 'b, 'c> Drop for Node<'a, 'b, 'c> {
	fn drop(&mut self) {
		if !::std::thread::panicking() {
			...
			let channel_monitor = test_utils::TestChainMonitor::new(Some(self.chain_source), self.tx_broadcaster.clone(), &self.logger, &feeest, &data_persister);
...
pub fn create_chanmon_cfgs(node_count: usize) -> Vec<TestChanMonCfg> {
	...
	for i in 0..node_count {
		...
		let data_persister = test_utils::TestChanDataPersister{};
		chan_mon_cfgs.push(TestChanMonCfg{ tx_broadcaster, fee_estimator, chain_source, logger, data_persister });
	}
...
pub fn create_node_cfgs<'a>(node_count: usize, chanmon_cfgs: &'a Vec<TestChanMonCfg>) -> Vec<NodeCfg<'a>> {
	...
	for i in 0..node_count {
		...
		let chain_monitor = test_utils::TestChainMonitor::new(Some(&chanmon_cfgs[i].chain_source), &chanmon_cfgs[i].tx_broadcaster, &chanmon_cfgs[i].logger, &chanmon_cfgs[i].fee_estimator, &chanmon_cfgs[i].data_persister);
