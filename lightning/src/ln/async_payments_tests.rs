...
use crate::blinded_path::payment::AsyncBolt12OfferContext;
...
#[test]
fn async_payments_reject_missing_invreq() {
...
#[test]
fn async_payments_reject_bad_payment_secret() {
...
fn expired_static_invoice_message_path() {
	...
	let path_expiry_height = crate::ln::inbound_payment::calculate_absolute_expiry(
		nodes[2].node.duration_since_epoch().as_secs(),
		INVOICE_EXPIRY_SECS,
	);
	let block =
		create_dummy_block(nodes[2].best_block_hash(), (path_expiry_height + 1) as u32, Vec::new());
