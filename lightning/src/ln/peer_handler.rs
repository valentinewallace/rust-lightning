...
struct PeerHolder<Descriptor: SocketDescriptor> {
	...
	// this mutex while we're processing a message - the top-level `peers` *write* lock must be
	// held instead to guarantee mutual exclusion of MessageHandler-visible events per-peer.
...
impl<Descriptor: SocketDescriptor, CM: Deref, RM: Deref, L: Deref, CMH: Deref> PeerManager<Descriptor, CM, RM, L, CMH> where
	...
	fn handle_message(
		...
		if let wire::Message::Init(msg) = message {
			log_trace!(self.logger, "Received Init message {:?} from {}", msg, log_pubkey!(their_node_id));
			...
			self.message_handler.route_handler.sync_routing_table(&their_node_id, &msg);
		...
		match msg {
			wire::Message::ChannelAnnouncement(ref msg) => {
				...
				for (_, peer_mutex) in peers.peers.iter() {
					...
					let encrypted_msg = peer.channel_encryptor.encrypt_message(&encoded_msg[..]);
