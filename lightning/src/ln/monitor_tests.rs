...
fn do_test_revoked_counterparty_commitment_balances(confirm_htlc_spend_first: bool) {
	...
	let expected_balance = vec![Balance::ClaimableAwaitingConfirmations { // to_remote output to B
	...
	assert_eq!(sorted_vec(vec![Balance::ClaimableAwaitingConfirmations { // to_remote output to B
			claimable_amount_satoshis: 100_000 - 5_000 - 4_000 - 3,
...
fn test_revoked_counterparty_htlc_tx_balances() {
	...
	assert_eq!(sorted_vec(vec![Balance::ClaimableAwaitingConfirmations { // to_self output to A
