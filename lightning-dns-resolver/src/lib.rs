...
mod test {
	...
	impl MessageRouter for DirectlyConnectedRouter {
		...
		fn create_blinded_paths<T: secp256k1::Signing + secp256k1::Verification>(
			...
			Ok(vec![BlindedMessagePath::one_hop(recipient, context, keys.get_inbound_payment_key(), &keys, secp_ctx).unwrap()])
		}
	}
	...
	async fn resolution_test() {
		...
		let reply_path =
			BlindedMessagePath::one_hop(payer_id, query_context, payer_keys.get_inbound_payment_key(), &*payer_keys, &secp_ctx).unwrap();
