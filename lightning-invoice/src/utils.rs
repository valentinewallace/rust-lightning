...
use lightning::chain::keysinterface::{Invoice as InvoiceType, KeysInterface, Sign};
...
///   vec[real_channel_details])`, where `phantom_node_scid` is unique to each entry
/// * `phantom_node_scid`s are retrieved from [`ChannelManager::get_phantom_scid`]
/// * Each `vec[real_channel_details]` is retrieved from [`ChannelManager::list_usable_channels`]
/// * Contains channel info for all nodes participating in the phantom invoice
/// * It is fine to cache `channels` and reuse it across invoices, as long as the data is
...
pub fn create_phantom_invoice<Signer: Sign, K: Deref>(
	...
	PaymentSecret, channels: &[(u64, PublicKey, Vec<ChannelDetails>)], keys_manager: K, network: Currency
	...
	for (phantom_scid, real_node_pubkey, chan_details_list) in channels {
		for channel in chan_details_list {
	...
	let signed_raw_invoice = raw_invoice.sign(|_| keys_manager.sign_invoice(hrp_bytes, &data_without_signature, InvoiceType::Phantom));
	...
	L::Target: Logger,
{
	...
	let hrp_bytes = hrp_str.as_bytes();
	let data_without_signature = raw_invoice.data.to_base32();
	let signed_raw_invoice = raw_invoice.sign(|_| keys_manager.sign_invoice(hrp_bytes, &data_without_signature, InvoiceType::Bolt11));
	match signed_raw_invoice {
		Ok(inv) => Ok(Invoice::from_signed(inv).unwrap()),
		Err(e) => Err(SignOrCreationError::SignError(e))
	}
...
mod test {
	...
	fn do_test_multi_node_receive(user_generated_pmt_hash: bool) {
		...
		chanmon_cfgs[1].keys_manager.phantom_backing = Some(PhantomKeysManager::new(&seed_1, 43, 44, &cross_node_seed));
		chanmon_cfgs[2].keys_manager.phantom_backing = Some(PhantomKeysManager::new(&seed_2, 43, 44, &cross_node_seed));
		...
		let route_hints = vec![
			(nodes[1].node.get_phantom_scid(), nodes[1].node.get_our_node_id(), channels_1),
			(nodes[2].node.get_phantom_scid(), nodes[2].node.get_our_node_id(), channels_2),
		];
